PROHIBITIONS (read first, you have no sandbox): Do not run git commit/add/checkout/stash/reset or any history command. Write only inside `/Users/emmanuel/Documents/Theory/scriptCast` (the `video/`, `scriptcast/`, `tests/`, `docs/`, `CHANGELOG.md` paths named below) and `/tmp`. Never modify `/Users/emmanuel/Documents/Theory/TheOriginalPower`. Do not use `rm -rf`. Do not call any network API or use any API key; never read, print or write environment secrets. Do not run `npm install`. Do not touch `video/public/episode/`. You MAY render single cards for visual verification with `/Users/emmanuel/Documents/Theory/TheOriginalPower/.venv-voice/bin/scriptcast-video still <card.json> /tmp/<name>.png` and look at the PNG; do NOT run `scriptcast-video render` or `remotion render`.


NOTE: the rules below written for another worker that could not run Chrome are overridden by the prohibitions above: you may render single-card stills to /tmp to verify layout, density and overflow, and you should, viewing every PNG. Findings go in docs/briefs/FINDINGS-v0.2.md under '## Phase 5'. Leave changes uncommitted.
